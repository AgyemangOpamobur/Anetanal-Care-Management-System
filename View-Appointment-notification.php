...
if (isset($_POST['search']))
{
    try {
      ...
      if (empty($searchid)) {
        echo "<script> alert('Please enter patient ID'); window.location.href='View-Appointment-notification.php';</script>";
      }else {
        if ($searchid)
        {
      ...
      echo "<script> alert('Incorrect ID / ID is not registered'); window.location.href='View-Appointment-notification.php';</script>";
    }
...
<head>
  <meta charset="utf-8">
  <title>View Appointment Notification</title>
</head>
...
<body>
  ...
  <form class="form-inline header-search-form my-2 my-lg-0"     action="View-Appointment-notification.php" method="POST" >
    <div id="page-wrap">
      <br>
    ...
    <br><br>
    <form class="" action="View-Appointment-notification.php" method="POST">
    <div id="page-wrap">
          ...
          <table border="1" width="200%"height="100" cellspacing="50" cellpadding="50">
            ...
            <tbody>
              ...
              <?php
                ...
                foreach ($conn->query("SELECT * from appointments where status ='Not Yet' order by today DESC ") as $row) {
              ?>
                <tr>
                ...
                <td><center>
                  ...
                  ?>
                          <a class="btn btn-sm btn-primary" href="View-Appointment-notification?del=Updated">Update</a></center></td>
                  <?php
                }else {
                   ?>
                  <a class="btn btn-sm btn-primary" href="View-Appointment-notification?update=<?php echo $row{'AID'}?>" >Update</a></center></td>
                  <?php
                }
                ?>
                <td><center><a class="btn btn-sm btn-primary" href="Send-Antenatal-SMS.php?id=<?php echo $row{'Mother_Id'}?>" >Send SMS</a> </center> </td>
                <?php
                  }
                }elseif($searchIDFlag == true)
                 {
                  $query = "SELECT * from appointments where status ='Not Yet' and  MRID='$actualId' order by today DESC";
                    $query_res = $pdo->query($query);
                    $count = count($query_res->fetchAll());
                    if ($count > 0)
                    {
                      foreach ($conn->query("SELECT * from appointments where status ='Not Yet' and MRID='$actualId' order by today DESC ") as $row) {
                    ?>
                      <tr>
                      <td><center><?php echo $row['Today']; ?></center></td>
                      <td><center><?php echo $row['Mother_Id']; ?></center></td>
                      ...
                      echo "<br><br>";
                    echo "<script> alert('Incorrect ID / ID has not been schedule for Antenatal Care'); window.location.href='View-Appointment-notification.php';</script>";
                    }
                }
               ?>
            </tr>
