...
try{
  ...
  $query = "SELECT * from menstrualhistory where MRID='$actualId'";
    $query_res = $pdo->query($query);
    $count = count($query_res->fetchAll());

    if ($count > 0)
     {
       header("location: History-search.php?recorderror");
     }else {
  ...
  if ($bol) {
   header("location: history.php");
    ...
    header("location: History-search.php?iderror");
...
<head>
  <meta charset="utf-8">
  <title>Antenatal History Search </title>
</head>
...
<br><br>
    <div class="col-xs-offset-3">
      ...
      if(isset($_GET['recorderror']))
      {
        ...
        <div class='alert alert-error'>
          <button class='close' data-dismiss='alert'>&times;</button>
          <strong><h4>There is existing records for this patient. Goto View Antentatal history page and update her records.</h4></strong>
        </div>
        <?php
      }
      ?>
    ...
    <br><br>
    <form class="" action="History-search.php" method="post">
    <div id="page-wrap">
      <table  width="180%" height="200" cellspacing="20" cellpadding="20">
          <thead>
            <tr>
              <th><center><h3 style="color: red;">Search</h3></center></th>
            </tr>
          </thead>
