...
//$pdf->AddFont()
 ...
 $pdf->Cell(190,5,"REGISTRATION CODE",0,1,'C');
 $pdf->ln();
...
$pdf->ln();
    ...
    foreach ($pdo->query("SELECT * from registrationcode where   staff_id={$_SESSION['staffid']}") as $row)
    {
      if ($row) {
        $pdf->SetFont('Helvetica','',12);
        $pdf->Cell(50,12,"",0);
