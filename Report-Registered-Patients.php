...
//generating records for year only
    ...
    if (empty($getYear1) && empty($getYear2)) {
      header("location: Report-Registered-Patients.php?dataerror");

      //checking if input is a string not integer
    }elseif(!ctype_digit(strval($getYear1)) && !ctype_digit(strval($getYear2))) {
      header("location: Report-Registered-Patients.php?dataerror");
    }else {
      $_SESSION['yearOne'] = $getYear1;
      $_SESSION['yearTwo'] = $getYear2;

      header("location: Pdf-Patient-Report-Year.php");
    }
  ...
  if (isset($_POST['generateMonth'])) {
    ...
    header("location: Pdf-Patient-Report-Month-Year.php");
...
<head>
  <meta charset="utf-8">
  <title>Report on Registered Patient</title>
</head>
...
<body>
  ...
  <form class="" action="Report-Registered-Patients.php" method="post">
...
<!-- Obstetric history -->
  <div id="page-wrap">
    <table  width="200%" height="400%" cellspacing="50" cellpadding="50">
        <thead>
          <tr>
            <th><center><h3 style="color: red; background-color: lightskyblue;   padding:20px;">GENERATE REGISTERED PATIENTS RECORD</h3></center></th>
          </tr>
        </thead>
        <tbody>
