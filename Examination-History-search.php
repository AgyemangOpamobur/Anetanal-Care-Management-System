...
if (isset($_POST['btn-Search']))
{
  ...
  $search = @trim(strtoupper($_POST['history_search']));
  $getID = explode("C", $search);
  $actualId = $getID[1];
  //checking if there is existing records for the patients
  $query = "SELECT * from physicalexaminationatfirstvisit where MRID='$actualId'";
    $query_res = $pdo->query($query);
    $count = count($query_res->fetchAll());

    if ($count > 0)
     {
       header("location: Examination-History-search.php?recorderror");
     }else {
  ...
  if ($bol) {
   header("location: Examination-history.php");
    ...
    header("location: Examination-History-search.php?iderror");
  }
 }
}catch(Exception $e){
  header("location: Examination-History-search.php?iderror");
...
<head>
  <meta charset="utf-8">
  <title>Examination History Search</title>
</head>
...
<br><br>
    <div class="col-xs-offset-3">
      ...
      if(isset($_GET['recorderror']))
      {
        ...
        <div class='alert alert-error'>
          <button class='close' data-dismiss='alert'>&times;</button>
          <strong><h4>There is existing records for this patient. Goto View Examination History page and update her records.</h4></strong>
        </div>
        <?php
      }
      ?>
    ...
    <br>
    <form class="" action="Examination-History-search.php" method="post">
    <div id="page-wrap">
      <table  width="180%" height="200" cellspacing="20" cellpadding="20">
          <thead>
            <tr>
              <th><center><h3 style="color: red;">Examination History Search</h3></center></th>
            </tr>
          </thead>
            ...
            <tbody>
              <label for="" class="control-label col-xs-8 " style="font-size: 18px;">Enter patient_ID here:</label><br>
                  ...
                  <tr>
                    <td>
                      <center>
                    <input id="bordershadow"type="text" name="history_search" value="" class="search col-xs-offset-3" style=" width:60%; height: 100%; font-size: 16px;" placeholder="Example: ANC1" required>
                  </center>
                    </td>
                  </tr>
                  <tr>
