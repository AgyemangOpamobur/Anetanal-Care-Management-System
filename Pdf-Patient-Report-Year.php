...
$query = "SELECT * from mothersrecord where year between {$_SESSION['yearOne']} and {$_SESSION['yearTwo']}";
  $query_res = $pdo->query($query);
  $count = count($query_res->fetchAll());
  if ($count > 0) {
    foreach ($pdo->query("SELECT * from mothersrecord where year between {$_SESSION['yearOne']} and {$_SESSION['yearTwo']}") as $row)
    {
      if ($row) {
        $int = array($row);
        @$value += count($int);
    ...
    header("location: Report-Registered-Patients.php?recorderror");
    exit();
  }
 // displays total record in the database
  ...
  $pdf->SetFont('Helvetica','B',11);
  $pdf->Cell(30,12,$_SESSION['yearOne'],0);
  $pdf->SetFont('Helvetica','',12);
  $pdf->Cell(15,12,"To:",0);
  $pdf->SetFont('Helvetica','B',11);
  $pdf->Cell(100,12,$_SESSION['yearTwo'],0);
  $pdf->ln();
  $text ="- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -";
