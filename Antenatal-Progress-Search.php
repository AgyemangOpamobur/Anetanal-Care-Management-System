...
try{
  $search = @trim(strtoupper($_POST['progress_search']));
  $getID = explode("C", $search);
  $actualId = $getID[1];

//checking if there is existing records for the patients
  $query = "SELECT * from antenatalprogressrecord where MRID='$actualId'";
  $query_res = $conn->query($query);
  $count = count($query_res->fetchAll());
  if ($count > 0)
    {
      header("location:  Antenatal-Progress-Search.php?recorderror");
    }else {
      ...
      if ($bol) {
         header("location: Antenatal-Progress.php");
     }else {
          header("location: Antenatal-Progress-Search.php?iderror");
      }
    }
    }catch(Exception $e){
      header("location: Antenatal-Progress-Search.php?iderror");
...
<head>
  <meta charset="utf-8">
  <title>Antenatal Progress Search</title>
</head>
  ...
  <center>
    ...
    <div class="col-xs-offset-3">
      ...
      if(isset($_GET['recorderror']))
      {
        ...
        <div class='alert alert-error'>
          <button class='close' data-dismiss='alert'>&times;</button>
          <strong><h4>There is existing records for this patient. Goto View Antenatal Progress page and update her records.</h4></strong>
        </div>
        <?php
      }
      ?>
    ...
    <br>
    <form class="" action="Antenatal-Progress-Search.php" method="post">
    <div id="page-wrap">
      <table  width="180%" height="200" cellspacing="20" cellpadding="20">
          <thead>
            <tr>
              <th><center><h3 style="color: red;">Antenatal Progress Search</h3></center></th>
            </tr>
          </thead>
            ...
            <tbody>
              <label for="" class="control-label col-xs-8 " style="font-size: 18px;">Enter patient_ID here:</label><br>
                  ...
                  <tr>
                    <td>
                      <center>
                    <input id="bordershadow"type="text" name="progress_search" value="" class="search col-xs-offset-3" style=" width:60%; height: 100%; font-size: 16px;" placeholder="Example: ANC1" required>
                  </center>
                    </td>
                  </tr>
                  <tr>
