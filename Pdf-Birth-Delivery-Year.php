...
$query = "SELECT * from summaryoflabourordeliveryoutcome where year between {$_SESSION['BirthyearOne']} and {$_SESSION['BirthyearTwo']}";
  $query_res = $pdo->query($query);
  $count = count($query_res->fetchAll());
  if ($count > 0) {
    foreach ($pdo->query("SELECT * from summaryoflabourordeliveryoutcome where year between {$_SESSION['BirthyearOne']} and {$_SESSION['BirthyearTwo']}") as $row)
    {
      if ($row) {
        $int = array($row);
        @$value += count($int);
    ...
    header("location: Report-Birth-Delivery.php?recorderror");
    exit();
  }
...
//counting total liver births
  $stmt = $pdo->query("SELECT Condition_of_Child from summaryoflabourordeliveryoutcome where Condition_of_Child='Live Birth'")->fetchAll(PDO::FETCH_COLUMN);
  $livebirth =   $stmt;
  @$liveBirth = count($livebirth);

  //counting total still birth births
    $stmt = $pdo->query("SELECT Condition_of_Child from summaryoflabourordeliveryoutcome where Condition_of_Child='Still Birth'")->fetchAll(PDO::FETCH_COLUMN);
    $stillbirth =   $stmt;
    @$stillBirth = count($stillbirth);

    //counting total still premature births
      $stmt = $pdo->query("SELECT Condition_of_Child from summaryoflabourordeliveryoutcome where Condition_of_Child='Premature'")->fetchAll(PDO::FETCH_COLUMN);
      $prematurebirth =   $stmt;
      @$prematureBirth = count($prematurebirth);
